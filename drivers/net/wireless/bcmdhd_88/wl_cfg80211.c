...
#ifndef CONFIG_BCMDHD88_DISABLE_P2P_SYSFS_DEVICE_NODE
...
#ifdef CONFIG_BCMDHD_CUSTOM_SYSFS_TEGRA
	{
		...
		if (status > 0) {
			WIFI_SCAN_DEBUG("%s: substituted wifi scan policy"
				" with %d rule(s)\n",
				__func__, status);
			TEGRA_SYSFS_HISTOGRAM_SCAN_CNT_INC(wl);
			return (0);
		} else if (status < 0) {
	...
	struct beacon_parameters *info)
{
	...
	if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
		...
		if (dev_role == NL80211_IFTYPE_AP) {
			...
			memcpy(wl->p2p->ssid.SSID, ssid_ie->data,
				wl->p2p->ssid.SSID_len);
		}
	}
...
scan_done_out:
	...
	if (wl->scan_request) {
#ifdef CONFIG_BCMDHD_CUSTOM_SYSFS_TEGRA
		TEGRA_SCAN_DONE(wl->scan_request, false)
#endif
...
#ifdef CONFIG_BCMDHD_CUSTOM_SYSFS_TEGRA
		TEGRA_SCAN_DONE(wl->scan_request, aborted)
#endif
...
static void wl_update_hidden_ap_ie(struct wl_bss_info *bi, u8 *ie_stream, u32 *ie_size)
{
	...
	if (ssidie[1] != ssid_len) {
		...
		memmove(ssidie + ssid_len + 2, ssidie + 2, *ie_size - (ssidie + 2 - ie_stream));
		memcpy(ssidie + 2, bi->SSID, ssid_len);
		*ie_size = *ie_size + ssid_len;
		ssidie[1] = ssid_len;
		return;
	}
	if (*(ssidie + 2) == '\0')
