...
int __init apalis_tk1_suspend_init(void)
{
	...
	if (pmu_board_info.board_id == BOARD_E1735) {
		struct tegra_suspend_platform_data *data = &apalis_tk1_suspend_data;
...
static struct soctherm_therm apalis_tk1_therm_pop[THERM_SIZE] = {
	[THERM_CPU] = {
		.zone_enable = true,
		.passive_delay = 1000,
		.hotspot_offset = 6000,
...
/* Only the diffs from apalis_tk1_soctherm_data structure */
...
int __init apalis_tk1_soctherm_init(void)
{
	...
	if (board_info.board_id == BOARD_E1923 ||
		...
		memcpy(apalis_tk1_soctherm_data.therm,
				apalis_tk1_therm_pop, sizeof(apalis_tk1_therm_pop));
	}
	...
	if ((cp_rev >= 0) && (ft_rev >= 0)) {
		tegra_platform_edp_init(
			apalis_tk1_soctherm_data.therm[therm_cpu].trips,
			&apalis_tk1_soctherm_data.therm[therm_cpu].num_trips,
			cpu_edp_temp_margin);
		tegra_platform_gpu_edp_init(
			apalis_tk1_soctherm_data.therm[THERM_GPU].trips,
			&apalis_tk1_soctherm_data.therm[THERM_GPU].num_trips,
			gpu_edp_temp_margin);
		...
		tegra_add_core_vmax_trips(
			apalis_tk1_soctherm_data.therm[THERM_PLL].trips,
			&apalis_tk1_soctherm_data.therm[THERM_PLL].num_trips);
	}
	...
	if ((pmu_board_info.board_id == BOARD_E1733) ||
		...
		apalis_tk1_soctherm_data.tshut_pmu_trip_data = &tpdata_palmas;
	else if (pmu_board_info.board_id == BOARD_E2174)
		apalis_tk1_soctherm_data.tshut_pmu_trip_data = &tpdata_max77620;
	else
